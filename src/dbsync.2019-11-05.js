...
define(['axios', 'underscore'], (axios, { isEqual }) => {
  ...
  const 
    ...
    _setFlag = (a, c, v) => (f => (h => h == !!v || (v && f.add(c) || h && f.delete(c)))(f.has(c)))(_get(a, 'flag') || _set(a, 'flag')),
  ...
  class Model extends Sync {
    ...
    set(attrs, val, opts) {
      ...
      _set(this, 'changed', new Map()) &&
      _set(this, 'preventAttrs', new Map(this.attrs)) ||
      (_has(this, 'changed') || _set(this, 'changed', new Map())) &&
      (_has(this, 'preventAttrs') || _set(this, 'preventAttrs', new Map()));
      ...
      Object.entries(attrs)
        ...
        .filter(([attr, val]) => {
          ...
          isEqual(_prev.get(attr), val) ?
            _changed.delete(attr) :
            _changed.set(attr, val);
