...
define(['axios', 'underscore'], (axios, { isEqual }) => {
  ...
  const 
    ...
    _prepareModel = (collection, attrs, opts) => {
      return attrs && 
        _isModel(attrs) &&
        (_has(attrs, 'collection') || _set(attrs, 'collection', collection)) && 
        attrs ||
        new collection.model(attrs, Object.assign({ collection }, opts));
    },
  ...
  class Model extends Sync {
    ...
    unset(attr, opts) {
      return this.set(attr, undefined, Object.assign({}, opts, { 'unset': true }));
    }
    ...
    set(attrs, val, opts) {
      ...
      const 
        { unset, changing } = opts,
        _privKeys = !this.isNew && new Set(privKeys.map(a => this[`${a}Attribute`])),
      ...
      Object.entries(attrs)
        ...
        .filter(([attr, val]) => {
          ...
          unset ? Reflect.deleteProperty(attrs, attr) : Reflect.set(_attrs, attr, val);
          
// console.log(Array.from(_prev));          
// console.log(Array.from(_changed));          
// console.log(Array.from(_attrs));          
// console.log('-.'.repeat(20));
    ...
    hasChanged(attr) {
      return (a => attr ? Reflect.has(a, attr) : !!Object.keys(a).length)(_get(this, 'changed'));
    }
  ...
  class Collection extends Sync {
    ...
    set(models, opts) {
      ...
      for (const model of Array.isArray(models) ? models : [models]) {
        ...
        if (existing) {
          ...
          const _model = _prepareModel(this, model, opts);
    ...
    create(model, opts) {
      return (model => model.save(null, opts)
        .then(model => this.add(model, opts) && model) 
      )(_prepareModel(this, model, opts));
    }
